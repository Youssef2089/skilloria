 ...
 * Finitions 2). Composant partagé entre /dashboard/freelance/missions/[id]
 * et /dashboard/cdi/missions/[id]. La logique métier est strictement
 * identique : seule la base path d'URL diffère (paramètre `side`).
 *
 * Charge le détail via GET /api/me/missions/[id] (qui flippe atomiquement
 * notif read + match notified→viewed côté serveur).
 *
 * Actions :
 *  - Candidater (POST /api/candidatures avec cover_message optionnel — la
 *    modal réutilisable CandidatureModal collecte le message).
...
export default function MissionDetailView({
  ...
  pubId: string | null
  /** SC7b : 'cdi' utilise /dashboard/cdi/missions comme base path. */
  side?: 'freelance' | 'cdi'
}) {
  ...
  return (
    <div style={{ maxWidth: 980, padding: '24px 26px' }}>
      ...
      {successBanner && (
        <div role="status" style={{ background: 'var(--sk-success-soft)', border: '1px solid var(--sk-success)', color: 'var(--sk-success)', padding: '12px 16px', borderRadius: 10, fontSize: 13, marginBottom: 18 }}>
          {successBanner}
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 14 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 12, color: 'var(--sk-muted)', marginBottom: 6, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontWeight: 500 }}>{tPub(`type.${pub.type}`)}</span>
            <span aria-hidden>·</span>
            <span>{orgName}</span>
            {pub.confidential && <span aria-hidden title={t('confidential_tooltip')}>🔒</span>}
          </div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--sk-text)', lineHeight: 1.25, letterSpacing: '-0.3px', marginBottom: 8 }}>
            {pub.title}
          </h1>
      ...
      <section style={{ background: 'var(--sk-surface)', border: '0.5px solid var(--sk-border)', borderRadius: 14, padding: '20px 22px', marginBottom: 18 }}>
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--sk-muted)', marginBottom: 12 }}>
          {t('section_description')}
        </div>
        <p style={{ fontSize: 14, color: 'var(--sk-text)', lineHeight: 1.7, whiteSpace: 'pre-wrap', margin: 0 }}>
          {pub.description}
        </p>
      </section>

      <section style={{ background: 'var(--sk-surface)', border: '0.5px solid var(--sk-border)', borderRadius: 14, padding: '20px 22px', marginBottom: 18 }}>
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--sk-muted)', marginBottom: 14 }}>
          {t('section_details')}
        </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'flex-end', marginTop: 24 }}>
        {!alreadyApplied && (
          <button
            ...
            disabled={dismissing}
            style={{
              padding: '12px 20px', background: 'transparent', color: 'var(--sk-muted)',
              border: '1px solid var(--sk-border)', borderRadius: 10,
              fontSize: 13, fontWeight: 600,
              cursor: dismissing ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
