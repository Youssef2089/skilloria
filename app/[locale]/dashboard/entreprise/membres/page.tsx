...
export default function MembresPage() {
  ...
  return (
    <div style={{ fontFamily: fontJakarta, display: 'flex', flexDirection: 'column', gap: 18, width: '100%', padding: '24px 26px 28px', boxSizing: 'border-box' }}>
      ...
      {toast && (
        <div role="status" style={{ position: 'fixed', bottom: 22, left: '50%', transform: 'translateX(-50%)', zIndex: 900, padding: '12px 20px', borderRadius: 12, fontSize: 14, fontWeight: 600, fontFamily: fontJakarta, color: 'var(--sk-surface)', background: toast.kind === 'error' ? 'var(--sk-red)' : 'var(--sk-success)', boxShadow: '0 10px 30px color-mix(in srgb, var(--sk-encre) 20%, transparent)' }}>
          {toast.msg}
        </div>
      )}
    </div>
