...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (prof.cv_hash === hash && prof.cv_parsing_status === 'done') {
    ...
    if (expRes.error || eduRes.error || langRes.error) {
      console.error('[cdi-upload-cv] cache ILLISIBLE — aucune réponse servie', {
        profileId: prof.id,
  ...
  if (branchRes.error || specialityRes.error || configRes.error) {
    console.error('[cdi-upload-cv] référentiel du domaine ILLISIBLE — aucun appel au modèle', {
  ...
  if (parsed.branch_slug) {
    ...
    if (brErr) {
      console.error('[cdi-upload-cv] branche ILLISIBLE — profil rattaché à aucune branche', {
  ...
  if (parsed.speciality_slugs.length > 0) {
    ...
    if (spsErr) {
      console.error('[cdi-upload-cv] spécialités ILLISIBLES — profil rattaché à aucune spécialité', {
  ...
  if (
    ...
    if (normalised.length === 0) {
      console.error(`[cdi-upload-cv] langues illisibles — aucune suppression`, { profileId: prof.id })
    } else {
      const { error: delErr } = await supabaseAdmin
  ...
  after(async () => {
    try {
      ...
      //    une cause lisible.
      if (postUpdErr) {
        console.error('[cdi-upload-cv] état post-analyse ILLISIBLE — mise en relation NON rejouée', {
          profileId: prof.id,
