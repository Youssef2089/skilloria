...
| Conservation des adresses IP et navigateurs (journaux d'audit et de session) | **12 mois** — une proposition, pas une mesure | `duree_reglages.conservation_ip_mois` | **Back-office** `/admin/durees` — **agit sur l'existant** : la tâche `ip_retention_purge` met à NULL, chaque nuit à 04:20, ce qui dépasse la durée, et laisse une trace par passage |
...
| `ip_retention_purge` | 04:20 | Met à NULL l'adresse IP **et** le navigateur des lignes d'audit et de session plus anciennes que la conservation réglée (§P3.4, 12 mois). **SQL pure, et elle écrit son propre verdict** dans `cron_run_log` par `cloturer_run_cron` — le même guichet que les tâches HTTP ; un échec est écrit sur la ligne, jamais levé. |
